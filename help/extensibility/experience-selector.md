...
> Servicio de API disponible> La API de GenStudio ofrece una API de RESTful para la administración de experiencias. Si desea usar la API de GenStudio para administrar experiencias, consulte la [documentación de la API de GenStudio](https://developer.adobe.com/genstudio-api/).
...
## Llamada de retorno de selección

Cuando se selecciona un(a) [!DNL Experience] en MFE y se hace clic en el botón **[!UICONTROL Usar]**, se debe invocar una función de devolución de llamada para pasar los detalles del(la) [!DNL Experience] seleccionado(a).

La llamada de retorno proporciona una cadena de HTML que representa a [!DNL Experience]. Si la experiencia incluye recursos (p. ej., imágenes, vídeos), los elementos de HTML, como `<img>` y `<video>`, incluirán `src` atributos que contienen direcciones URL prefirmadas para estos recursos. Estas direcciones URL firmadas previamente tienen un tiempo de caducidad de 30 minutos y la aplicación cliente debe consumir [!DNL Experience] en ese tiempo.

### Esquema de salida

La llamada de retorno `onSelectionConfirmed` recibe una matriz de [!DNL Experience] objetos con la siguiente estructura:

| Propiedad | Tipo | Descripción |
|----------|------|-------------|
| `id` | `string` | Identificador único de [!DNL Experience] (formato URN). |
| `content` | `string` | Cadena HTML codificada en Base64 que representa [!DNL Experience]. |
| `experienceFields` | `object` | Pares de clave-valor de campos editables en [!DNL Experience]. Cada campo de `experienceFields` contiene: <ul><li>`fieldName`: nombre/identificador del campo.</li><li>`fieldValue`: el contenido/valor real del campo.</li></ul> |
| `metadata` | `object` | Metadatos sobre la experiencia (por ejemplo: `channel`, `externalTemplateMetadata`, `externalAssetMetadata`, etc.) |
| `aspectVariants` | `array` | Matriz de variantes de proporción de aspecto para [!DNL Experience]. Cada variante contiene: <ul><li>`aspectKey`: Identificador único de la variante.</li><li>`aspectMetadata`: metadatos, incluidos `channel` y `aspectRatio`.</li><li>`content`: cadena HTML codificada en Base64 para esta proporción de aspecto específica.</li></ul> |

### Ejemplo JSON

A continuación se muestra un bloque JSON de muestra que muestra los esquemas.
